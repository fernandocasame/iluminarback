...
class MatriculaController extends Controller
{
    ...
    public function procesar_pagos(Request $request) // excel
    {
        ...
        for( $i=0; $i<count($pagos); $i++ ){
            $cuota = DB::SELECT("SELECT u.idusuario, em.user_created, 
            em.id_matricula, cxc.id_cuotas_id, cxc.num_cuota, 
            cxc.valor_cuota, cxc.valor_pendiente
            FROM usuario u, mat_estudiantes_matriculados em,
            periodoescolar p, mat_cuotas_por_cobrar cxc
            WHERE u.cedula = ?
            AND em.id_estudiante = u.idusuario 
            AND em.id_periodo = p.idperiodoescolar
            AND p.estado = '1'
            AND em.id_matricula = cxc.id_matricula
            AND cxc.valor_pendiente > 0
            AND cxc.num_cuota > ?
            ORDER BY cxc.num_cuota LIMIT 1", [$pagos[$i]->cedula,$request->tipo]);
            if( $cuota ){
                $forma_pago_excel = [
                    ...
                    'id_usuario' => $cuota[0]->user_created,
                    'num_cuota' => $cuota[0]->num_cuota
                ];
                $cuotas_pendientes = DB::SELECT("SELECT *
                FROM `mat_cuotas_por_cobrar`
                WHERE `id_matricula` = ?
                AND `num_cuota` > ?", [$cuota[0]->id_matricula,$request->tipo]);
                $total_pendiente = 0;
                foreach ($cuotas_pendientes as $key => $value) {
                    $total_pendiente = $total_pendiente + $value->valor_pendiente;
                }
                ...
                for( $j=0; $j<15; $j++ ){ // 15 modificar si aumenta el numero de cuotas posibles
                    if( $sobrante > 0 ){
                        $cuota = DB::SELECT("SELECT * FROM `mat_cuotas_por_cobrar`
                        WHERE `id_matricula` = ? AND `num_cuota` = ?",
                        [$forma_pago_excel['id_matricula'], ($forma_pago_excel['num_cuota']+$j)]);
                        if( $cuota ){
                            if( $sobrante >= $cuota[0]->valor_pendiente ){
                                $sobrante = $sobrante - $cuota[0]->valor_pendiente;
                                $saldo_actual = 0;
                            ...
                            $pago = DB::INSERT("INSERT INTO 
                            `mat_pagos`(`id_cuota`,`forma_pago`,`id_usuario`)
                            VALUES (?,?,?)", [$cuota[0]->id_cuotas_id, $forma_pago, $forma_pago_excel['id_usuario']]);
                            if( $pago ){
                                DB::UPDATE("UPDATE `mat_cuotas_por_cobrar`
                                SET `valor_pendiente` = ? WHERE `id_cuotas_id` = ?",
                                [$saldo_actual,$cuota[0]->id_cuotas_id]);
                            }
                        }
                    }
                }
    ...
    {
        $cuota = DB::UPDATE("UPDATE `mat_cuotas_por_cobrar` SET `valor_cuota`= ?, `valor_pendiente`= ?, `user_created`= ? WHERE `id_cuotas_id` = ? ", [$request->valor_cuota, $request->valor_pendiente, $request->id_usuario, $request->id_cuota_edit]);
